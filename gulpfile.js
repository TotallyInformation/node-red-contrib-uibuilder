...
async function buildFeModule() {
    ...
    const fileContent = await readFile(entryPoint, 'utf8')
    ...
    if (v[1] !== release) {
        ...
        // await writeFile(entryPoint, fileContent.replace(/const version = '(.*)-src'/, `const version = '${release}-src'`), 'utf8')
    }
    ...
    try { // IIFE minified
        ...
        await writeFile(outFilePath, fileContent.replace(/version = "(.*)-src"/, 'version = "$1-iife.min"'), 'utf8')
    } catch (e) {
    ...
    try { // IIFE not-minified
        ...
        // Update the output version string if build was successful
        const fileContent = await readFile(outFilePath, 'utf8')
        await writeFile(outFilePath, fileContent.replace(/version = "(.*)-src"/, 'version = "$1-iife"'), 'utf8')
    } catch (e) {
    ...
    try { // ESM minified
        ...
        // Update the output version string if build was successful
        const fileContent = await readFile(outFilePath, 'utf8')
        await writeFile(outFilePath, fileContent.replace(/version = "(.*)-src"/, 'version = "$1-esm.min"'), 'utf8')
    } catch (e) {
    ...
    try { // ESM not-minified
        ...
        // Update the output version string if build was successful
        const fileContent = await readFile(outFilePath, 'utf8')
        await writeFile(outFilePath, fileContent.replace(/version = "(.*)-src"/, 'version = "$1-esm"'), 'utf8')
    } catch (e) {
...
async function buildUiModule() {
    ...
    try { // ESBuild ui.mjs as a Node.js cjs library
        ...
        // Update the output version string if build was successful
        const fileContent = await readFile(outFilePath, 'utf8')
        await writeFile(outFilePath, fileContent.replace(/version = "(.*)-src"/, 'version = "$1-node"'), 'utf8')
    } catch (e) {
...
async function buildModule(config) {
    ...
    if (v[1] !== release) {
        ...
        // await writeFile(entryPoint, fileContent.replace(/static version = '(.*)-src'/, `static version = '${release}-src'`), 'utf8')
    }
...
async function buildExperimentalModule() {
    ...
    try { // ESM not-minified
        ...
        // Update the output version string if build was successful
        const fileContent = await readFile(outFilePath, 'utf8')
        await writeFile(outFilePath, fileContent.replace(config.versionSource, config.versionTarget), 'utf8')
    } catch (e) {
        throw new Error(`[${config.name} Library] ESM (unminified) Build failed. ${e.message}`)
    }
}
