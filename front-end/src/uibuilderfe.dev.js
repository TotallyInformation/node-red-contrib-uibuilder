...
(function () {
    ...
    var uibuilder = (function () {
        ...
        */
         ...
         * The incoming msg must contain either msg._uib.props or msg.payload.
         * If props/payload is not an object, a raw v-bind wont work, you have to be setting a specific prop in that case.
         * e.g. <button-counter ref="cb1" v-bind="cb1props"></button-counter> would need props to be an object
         * or <button-counter ref="cb2" v-bind:varient="cb2props" tweak="jim"></button-counter> would need props to be a value
         * This cannot be validated in the watch function, user beware.
         * NOTE: Deep objects in props might not work.
         * @param {string} watchTopic msg.topic to watch for
         * @param {*} varRef Reference to the variable to be changed - this will be forced to be Vue responsive (to the first level if an object)
         */
        self.watch = function(watchTopic, varRef) {
            // Only if Vue is in use and a reference to the Vue master app is available ...
            if ( !self.vueApp ) {
                self.uiDebug('warn', '[uibuilder:watch] Reference to Vue not available (see `start` function) or Vue not in use. Watch function requires access to a Vue app.')
                return
            }

            if ( varRef === null || varRef === undefined ) {
                console.error(`[uibuilder:watch:onChange ${watchTopic}] varRef parameter must not be undefined or null.`)
                return    
            }
            ...
            self.onChange('msg', function(msg) {
                if ( msg.topic !== watchTopic ) return

                console.log(`watch:onChange :: ${watchTopic}`, varRef, msg.payload, msg._uib, self.vueApp)

                // Try to get some props otherwise give up with a warning
                let props
                if ( msg._uib ) {
                    if ( msg._uib.props ) props = msg._uib.props
                } 
                if ( msg.payload && props === undefined ) {
                    props = msg.payload
                }
                ...
                // Set the given Vue data variable. Walk the top-level properties to ensure that the Vue data var is responsive
                Object.keys(props).forEach( key => {
                    self.vueApp.$set(varRef, key, props[key])
                    console.log(key, props[key])
                    //self.vueApp.$set(self.vueApp.data[varRef], key, props[key])
                })

                // if ( props.constructor.name === "Object" ) {
                //     if ( varRef === undefined ) {
                //         varRef = {}
                //         console.log('initialise var', varRef)
                //     }
                    
                //     Object.keys(props).forEach( key => {
                //         //self.vueApp.$set(varRef, key, props[key])
                //         self.vueApp.$set(self.vueApp[varRef], key, props[key])
                //     })
                // } else {
                //     // Vue does not allow dynamically adding new root-level reactive properties to an already created instance - it must already exist for this to work
                //     //varRef = props
                //     self.vueApp[varRef] = props
                //     if ( msg.topic === 'alert3props' ) {
                //         //self.vueApp.alert3props = props
                //         console.log('alert3', props, self.vueApp.alert3props)
                //     }
                // }
        ...
        self.uiReturn = {
            ...
            /** Create a "watch" that matches an incoming msg against a specified variable
             *  and updates it automatically
             * @param {string} watchTopic Optional. msg.topic text to be watched for
             * @param {*} varRef Reference to the variable to be changed
             */
            watch: self.watch,
            /** Create a "watch" that matches an incoming msg against a specified variable
             *  and updates it automatically
             ...
             * @param {*} varRef Reference to the variable to be changed
             */
            // watch: function(watchTopic, varRef) {
            //     self.onChange('msg', function(msg) {
            //         console.log(`watch:onChange`, varRef, msg.payload)
            //         if ( msg.topic === watchTopic ) {
            //             console.log(`watch:onChange :: ${watchTopic}`, varRef, msg.payload)
            //         }
            //     })
            // }
