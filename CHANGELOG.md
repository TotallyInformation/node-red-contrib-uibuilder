...
* Move all `libs\*.js` to `src/libs/` and use gulp watch to build (compress and force to correct node.js version)
* `package-mgt.js`
...
* The `ui.js` library now added as a node.js library to `libs/ui.js` in preparation for using it in a future node. Auto-generated by `gulp watch` when the source is changed.
* Source-code for the uibuilder runtime support libraries has been moved to `src/libs`. Gulp watch is used with ESBUILD to enforce correct node.js version support and minify the libraries. This may reduce uibuilder's memory footprint & improve startup time on limited devices.
